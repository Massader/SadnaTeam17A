...
const SignIn = ({ onLogin, onRegister, newClientCredentials }: Props) => {
  const { clientCredentials, setClientCredentials } = useContext(
    ClientCredentialsContext
  );

  const { username, setUsername } = useContext(ClientCredentialsContext);

  const { isAdmin, setAdmin } = useContext(ClientCredentialsContext);

  const { roles, setRoles } = useContext(ClientCredentialsContext);
    ...
    <Flex padding={10} justifyContent="center" alignItems="center">
      <Stack spacing={4} w="100%" maxW="400px" px={4}>
        ...
        <Flex justifyContent="space-between" alignItems="center">
          <Button onClick={onRegister} fontSize="sm" fontWeight="bold">
            REGISTER
          </Button>
